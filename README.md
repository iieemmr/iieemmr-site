...
Shared, reusable pieces live in `components/shared/`: `BeforeAfterSlider` (compare-slider for the Then & Now section, dynamically imported client-side via `BeforeAfterSliderClient` with `ssr: false`), `VideoCarousel` (embla-carousel-powered slider over multiple `VideoEmbed`s, used by `VideoHighlight`), `VideoEmbed` (video player wrapper built on `VideoMedia`; renders a placeholder until a real video URL is supplied), `VideoMedia` (renders a YouTube iframe or `<video>` element depending on the URL, using each platform's native playback controls; lazy-loads via `useInView` so the real embed only mounts once scrolled into view, showing the video's YouTube thumbnail as a placeholder until then — `VideoCarousel` shares one `useInView` check across all its slides so they load together instead of each needing its own visibility crossing), `PlaceholderBox` (generic labeled placeholder for logos/photos), `LogoGrid`, `AlbumGrid` (album-cover grid + Load More/Show Less for `PhotoGallery`, opens `PhotoLightbox` for the clicked album), `AlbumCard` (single grid cell rendered by `AlbumGrid`; on hover, cross-fades through a few of that album's photos with a subtle Ken Burns zoom), `PhotoLightbox` (per-album modal with Escape/click-outside/arrow-key navigation; shows a pulsing skeleton over the active photo while it loads and eagerly preloads the next/prev photo in the background so navigating feels instant), `PhotoThumbnailStrip` (embla-carousel-powered thumbnail strip inside `PhotoLightbox` for jumping to a specific photo in the open album), and `ShareButton` (native share sheet with clipboard-copy fallback).

`app/robots.ts`, `app/sitemap.ts`, `app/not-found.tsx`, and `app/error.tsx` provide the site's SEO and error-handling essentials; the latter two are branded and wrapped in `SiteNav`/`SiteFooter` automatically via the root layout. `lib/structured-data.ts` exports `eventJsonLd`, a schema.org `Event` object rendered as a `<script type="application/ld+json">` in `app/page.tsx` for search-engine rich results.

`app/icon.png` and `app/apple-icon.png` are the browser tab icon and iOS home-screen icon (Next.js's file-convention icons — no code wiring needed). `app/favicon.ico` is a 32×32 render of the same logo wrapped in a minimal PNG-based `.ico` container, served at the legacy `/favicon.ico` path some crawlers and older tools check directly instead of reading the `<link rel="icon">` tag. The full-resolution source logo lives at `public/brand/iiee-logo.png`, for any future branding use beyond the icons.
