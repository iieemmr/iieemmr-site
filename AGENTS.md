...
- [x] Decide client's Vercel access — decided 2026-08-31: create a new Team (not the personal Hobby account) named for the client's org, upgrade it to Pro ($20/mo), transfer the `iieemmr-site` project into it, and invite the client as a free **Viewer** seat (read-only — deploys/analytics visibility, no publish/settings access). This is a deliberate cost tradeoff, not a full ownership handoff: Vercel's Pro pricing is $20/mo for 1 included paid seat (Owner or Member role) + $20/mo per *additional* Owner/Member/Developer seat, with Viewer and Billing roles free. A genuine handoff (client holding Owner) would require the client to occupy a paid seat too — 2 paid seats, $40/mo total — since Owner is billed the same as Member regardless of who created the team. Chose $20/mo: dev (paulfuentesss) stays the sole paid seat/Owner, client stays Viewer-only.
- [x] Vercel plan — resolved by the above: moving off the personal Hobby account to a Pro Team addresses both the ToS commercial-use concern and the client-access requirement in one step.
...
Costs: domain renewal stays with the client at Wix ($12.90/yr, currently auto-renew off — see above); GitHub is free (public repo); Vercel is either free (Hobby, ToS gray area for a live business site) or $20/mo per Developer seat on Pro (recommended) — a second Developer seat (another $20/mo) is only needed if the client wants hands-on deploy access themselves rather than just Viewer visibility.
